...
for /f "tokens=1,2 usebackq delims=." %%a in (`dir  /o:n /b`) do (
	...
	if not "%%b"=="bat" (
		if "%%b"=="html" (
			echo ^<li^>^<a href="53/%%a.%%b"^>%%a^</a^>^</li^> >>more.html 
		) else (
			echo ^<li^>^<a href="53/%%a.%%b"^>%%a.^<span style="color:red; "^>%%b^</span^>^</a^>^</li^> >>more.html
		)
	)
	)
	)
...
for /f "tokens=1,2 usebackq delims=." %%a in (`dir  /o:n /b`) do (
	if "%%a.%%b"=="%%a." (
		echo ^<li^>^<a href="53/%%a/a.html"^>^<span style="color:blue; "^>%%a^</span^>^</a^>^</li^> >>more.html
	)
