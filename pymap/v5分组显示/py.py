    ...
    <style>
        * { margin: 0; padding: 0; }
        ...
        #color-bar .bar {
            ...
            margin: 0 8px;
            border-radius: 3px;
            background: linear-gradient(to right, #2b88b9, #b8c418, #e8943a, #df6a5d);
        }
    </style>
</head>
<body>
    ...
    <script>
        ...
        var capitalMap = {
            ...
            '河南':'郑州','河北':'石家庄','山东':'济南','山西':'太原',
            '四川':'成都','西藏':'拉萨','云南':'昆明','贵州':'贵阳',
            '吉林':'长春','安徽':'合肥',
            '北京':'北京','天津':'天津','上海':'上海','重庆':'重庆',
            '内蒙古':'呼和浩特',
            '青海':'西宁','甘肃':'兰州','陕西':'西安',
            '江苏':'南京','江西':'南昌','浙江':'杭州','黑龙江':'哈尔滨',
            '宁夏':'银川','新疆':'乌鲁木齐',
            '辽宁':'沈阳','福建':'福州','海南':'海口','台湾':'台北'
        };
        ...
        var provinceInfo = [
            ...
            {name:'吉林',group:2},{name:'安徽',group:2},
        ...
        function getDisplayData() {
            return provinceInfo.map(function(p) {
                var color = groupState[p.group] ? '#ffffff' : groupColors[p.group];
                return {
                    name: p.name,
                    value: p.group * 25,
                    itemStyle: { areaColor: color, borderColor: groupState[p.group] ? '#ccc' : '#888' }
                };
            });
        }
        ...
        var option = {
            ...
            tooltip: {
                ...
                formatter: function(params) {
                    ...
                    var capital = capitalMap[province] || '';
                    return province + '<br/>省会：' + capital;
                }
            },
            toolbox: {
                show: true,
