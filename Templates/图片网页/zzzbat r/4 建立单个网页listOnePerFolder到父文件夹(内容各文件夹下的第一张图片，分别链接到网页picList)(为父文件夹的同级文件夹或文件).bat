...
echo div{width:88.125^%%;} >>..\listOnePerFolder.html
echo div{margin^:auto^;} >>..\listOnePerFolder.html
echo img{display^:block^;margin^:auto;} >>..\listOnePerFolder.html
echo img{padding^:2px 2px; height^:300px; float^:left;} >>..\listOnePerFolder.html
...
for /f "tokens=1,2 usebackq delims=." %%a in (`dir ..\ /o:d /b`) do (
	...
	if not "%%n"=="listOnePerFolder.html" (
		echo ^<a href="%%a/picList.html"^>^<img src="%%a/01.jpg" /^>^</a^> >>..\listOnePerFolder.html
	)
	)
	)
)
