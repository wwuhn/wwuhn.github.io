...
echo div{width:94.125^%;} >>docList.html
echo div{margin^:auto^;} >>docList.html
echo img{display^:block^;margin^:auto;} >>docList.html
echo img{padding^:2px 22px;} >>docList.html
echo img{border^:0 red solid;} >>docList.html
echo a:link{text-decoration: none; color:#fff;} >>docList.html
...
for /f "tokens=1,2 usebackq delims=." %%b in (`dir /o:d /b`) do (
	if not "%%c" == "bat" (
		...
		echo ^<li^>^<a href="%%b.%%c" title="%%b.%%c"^>^%%b^</a^>^</li^> >>docList.html
	)
