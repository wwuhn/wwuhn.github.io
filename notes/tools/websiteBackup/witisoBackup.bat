...
copy  %~dp0witiso\* %userprofile%\desktop\witiso


for /f "tokens=1,2 usebackq delims=." %%a in (`dir %~dp0witiso /o:d /b`) do (
	if "%%a.%%b"=="%%a." (
		md %userprofile%\desktop\witiso\%%a
	)

)
for /f "tokens=1,2 usebackq delims=." %%j in (`dir %~dp0witiso /o:d /b`) do (
	if "%%j.%%k"=="%%j." (
