...
/*
从代码中找其原因。解决冲突中重新定址的adr=(adr+1)%p应该是adr=(adr+1)%m，应该以存储单元
数m为依据，决定是否将下标返回到0。
　　本文开头的课件截图中，也明确示出 di=(di?1+1)　mod　mdi=(di?1+1)　mod　m 。相关的I
nsertHT函数和SearchHT函数中对应的代码，都应该改过。在修改中，存储单元数m也应该作为参数
传递给相关的函数。
　　感谢烟台大学计146-2班杨珺同学指出错误。本文讨论至此，暂不做修改。这可能更有利于读
者从我的错误中得到提高。
