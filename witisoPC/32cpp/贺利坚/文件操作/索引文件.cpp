...
(2)输出主文件中的学生记录
(3)建立与主文件相对应的索引文件，其中每个记录由两个字段组成：学号和该学生记录在数据文
件中的位移量（例：上面列出数据中学号为2的学生许可，其位移量是5。由于本例用定长文件，可
以根据位移量计算记录在主文件中的相对地址）。索引文件按学号有序。
(4)输出索引文件中的全部记录
(5)根据用户输入的学号，利用索引文件，用二分查找法找到对应的记录号，再通过主文件输出该
记录
